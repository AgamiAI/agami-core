...
**These are enforced, not just guidance.** Every query runs through a read-only gate (`sql_guard`, at the executor chokepoint) that rejects anything other than a single `SELECT` / `WITH...SELECT` — multi-statement SQL, data-modifying CTEs, transaction-control / session-state / prepared statements, `SELECT ... INTO`, row-level locks, and dangerous server-side functions on every engine it serves (`pg_read_file`, `lo_export`, `dblink`, `copy_program`, `pg_sleep`, `pg_notify`, advisory locks, `query_to_xml`, and their equivalents elsewhere — `SLEEP`, `BENCHMARK`, `GET_LOCK`, `LOAD_FILE`, `SYSTEM$…`, `reflect`, `OPENROWSET`, `OPENQUERY`, `EXTERNAL_QUERY`, `UTL_HTTP.*`, …). A rejected query never reaches the database. Generate read-only SQL and this gate stays invisible.
