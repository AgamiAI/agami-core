...
| `reconcile/<ts>/` | One reconcile run: `intake.json` (the run's own copy of what runs), `intake.html`, `rows.jsonl`, `report-items.json`, `report.html`, `ledger.json`, `findings.json`, `query_defects.json`, and `rows/<n>/` holding each supplied statement, its run record, receipts, probe plans and probe CSVs. Question text and SQL, never result rows beyond one recorded cell. See [`part-ledger.md`](part-ledger.md) |
