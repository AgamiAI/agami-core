...
| `literal:<t>.<c>=<v>` | `filter-values.judge.json` | the judge's grade, as it stands; a `model_gap` is of kind `description`, the column's list of values being stale. Every grade carries `declared` (`populated`, `empty`, `absent`), and a grade the warehouse decided over an undeclared column says so in its note The existence count decides over the distinct list's spelling: a value the list does not spell but the statement's own predicate matches rows for is `confirmed`, the note naming the column's spelling; the list alone grades a defect only when the count was zero or the probe did not run. |
