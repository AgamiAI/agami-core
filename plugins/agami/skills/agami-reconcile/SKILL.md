...
You are running the reconciliation harness. Goal: take the evidence a person brings — labeled numbers from an existing dashboard (Tableau / Looker / Mode / Metabase / Power BI / spreadsheet), most often a **screenshot**, sometimes a CSV or pasted list; or the SQL they trust; or a list of questions — and prove agami can reproduce each answer. When numbers match and every part of the person's statement checks out, that's evidence the semantic model is right. When they don't, the receipt drill-down and the part ledger explain why — a definitional disagreement (gross vs net, refunds in vs out, FX rate at booking vs reporting date), a join the semantic model is missing, or a mistake in the person's own query — which is exactly the trust signal that makes a DE relax.
...
- **1.5b — Run it the way agami runs its own.** `sm prepare` first, then the profile's tier exactly as `agami-query` Phase 1e tabulates it (psql, mysql, snowsql, sqlite3, DuckDB, or `execute_sql`), never `--no-safety`. Write stdout to `statement.csv` and the exit and stderr into `run.json`. **A refusal is a finding, not a crash**: a `table_scope` or `column_scope` refusal grades `scope: model_gap`, because the person wanted a table or column the semantic model does not expose; `select_star` grades `runs: query_defect`. Never rewrite the statement and never retry. Other failures go through [`shared/db_error_classifier.md`](../../shared/db_error_classifier.md); `auth`, `dsn`, `network` and `permission` stop the run as `agami-query` Phase 3b stops it. Nothing in this phase writes `query_log.jsonl`: `agami-save-correction` reads that log's last successful line as the question to correct, and a probe there would be corrected instead of the answer.
- **1.5c — Its receipt.** `sm receipt "$ROOT" --sql-file statement.sql > statement-receipt.json` whenever the statement parsed.
- **1.5d — Probes.** `sm join-probes` and `sm filter-values plan` emit SQL; run every emitted probe through the same tier, each to the CSV `part-ledger.md` names, then `sm filter-values judge`. A probe the tier refuses or fails leaves an empty file; leave it there, the ledger reads it as a probe that failed.
...
The seven claims (tables, filter predicates, date window, group keys, join keys, ordering, limit) say which part differs; they never say who is right. Then set the row's `status` with `reconcile.py status`, from the diff's `match` and the ledger's verdict.
