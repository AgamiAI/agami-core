...
**It probes as much as it likes, and a query that did not answer ends the row.** Successful queries are free, and how many a row needed is the measurement: `probe_count` reads how much the semantic model failed to say up front. But the first query the database rejects or the safety check blocks stops the row, and the row's `error` says in plain words what stopped it and why, rather than a fixed sentence. That is not a run that went wrong, it is the finding: a statement that did not run says the semantic model names something that is not there, and a client left free to retry would bury exactly that. A tool that crashed is different: that is agami's own tool failing, not the semantic model, and the row is one to ask again. The rule is enforced by the server, so it cannot be half-obeyed.
...
```json
{
  ...
  "probes":       [<the client's tool calls, in order, as the server recorded them>]  // only under --via mcp, where there is a trace; absent otherwise
