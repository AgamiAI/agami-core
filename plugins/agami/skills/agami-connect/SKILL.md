...
2. **The one-time exception to "never ask in chat":** during Phase 0a (the inline credentials-collection flow that runs when no credentials exist for the active profile), this skill asks the user **once** to paste a connection URL or a SQLite path. That's the only authorized credential prompt in the whole flow. Once written to `~/.agami/credentials`, never re-ask in chat — that file is the contract.
...
4. **If credentials are missing for the active profile, run Phase 0a inline** (do NOT bail out to a separate skill). Phase 0a writes the credentials file from a single user-pasted URL, then continues into Phase 1. Use `/agami-init verify` only as the *post-setup verification surface* — the primary onboarding entry is this skill.
5. **NEVER put the password (or any credential field) in a Bash command line.** That includes `export PGPASSWORD='<value>'`, `export MYSQL_PWD='<value>'`, `psql -W <password>`, `mysql -p<password>`, or any heredoc form that interpolates the password into stdin. Hosts render Bash tool calls in chat — anything in the command leaks. **Phase 0a writes credentials via the Write tool**, never via Bash echo / heredoc. Use the auth files generated by `scripts/setup_pgauth.py` for runtime queries: `PGPASSFILE=$HOME/.agami/.pgpass psql -h ... -U ... -d ... -c "$SQL" --csv` (psql) or `mysql --defaults-file=$HOME/.agami/.mysql.cnf --defaults-group-suffix=_<profile> ...` (mysql). For the Python driver path use `python3 scripts/execute_sql.py`. See [`shared/connection-reference.md → HARD RULES`](../../shared/connection-reference.md).

If you find yourself reaching for any command that doesn't fit the rules above, stop and re-read this section.

### Preflight steps

1. **Bootstrap `~/.agami/`** if missing: `mkdir -p ~/.agami && chmod 700 ~/.agami`. (This was previously agami-init's job; agami-connect now owns the full first-run path.)
2. **Resolve `<profile>`** in this order: `AGAMI_PROFILE` env var → `active_profile` field in `~/.agami/.config` → literal string `"main"` (the new default; old installs may still have `"default"` and that continues to work). The OSI `semantic_model[].name` MUST equal the resolved `<profile>`.
3. **Credentials check.** Read `~/.agami/credentials` if present and look for the `[<profile>]` section. If the file is missing, OR the section for the active profile is missing, OR `AGAMI_DATABASE_URL` is unset → run **Phase 0a** (inline collection) before continuing. If credentials exist, apply the chmod check (refuse if world-readable) and skip Phase 0a.
...
5. **Tool detection.** Look up the cached connection method and tool paths from `~/.agami/.config`. If absent, run tool detection per [`agami-init/SKILL.md → Phase 3 (tool detection)`](../agami-init/SKILL.md#phase-3-tool-detection) — the detection logic still lives there and is invoked from here on first-run.
...
7. **Update-check (best-effort, non-blocking).** Run the version probe from [`shared/version-check.md`](../../shared/version-check.md). If a newer plugin version exists on `main`, surface a one-line note ("agami X.Y.Z is available — run `/plugin marketplace update litebi && /reload-plugins`"). Never block on a network failure or stale local file — the probe is informational only.
8. If `$ARGUMENTS` is `reintrospect`: skip Phase 1's "already-have-a-model?" check and re-introspect from scratch. **Hand-edits the user made (descriptions, ai_context, choice_fields, metrics, trust-layer sign-offs) MUST be preserved** — re-introspection only updates what the DB unambiguously tells us (table list, columns, types, PK, FK).

---

## Phase 0a: Inline credentials collection (only when credentials are missing)

This is the first-run path. Goal: in **one user turn**, collect the connection, write the credentials file, materialize the provider-native auth files, and continue into introspect.

### 0a.1 — Ask for the connection (one question, free-text)

Surface a single prompt — no AskUserQuestion modal here, just plain chat:

> Paste your database connection. Two acceptable shapes:
>
> - **Connection URL** (most cloud DBs hand you one): `postgresql://...`, `postgres://...`, `mysql://...`, `redshift://...`, `snowflake://user:pass@account/db?warehouse=W`, or `sqlite:///absolute/path.db`. The `+driver` suffixes (asyncpg, psycopg2, pymysql) are stripped automatically.
> - **SQLite file path** (absolute or `~`-prefixed): e.g. `~/Documents/data/shop.db` — works without a URL scheme.
>
> Got a Cloud SQL Auth Proxy command instead? Run that in another terminal first, then paste the local URL it lets you connect to (typically `postgresql://USER:PASS@127.0.0.1:<port>/DB`).

**Default profile name is `main`** — don't ask. The user can rename via `/agami-init switch <new-name>` later. (Old installs may have `default`; both work.)

### 0a.2 — Detect the input kind

Branch on the user's pasted string (after stripping whitespace):

- **URL with a known scheme** (`postgresql://`, `postgres://`, `postgresql+<driver>://`, `mysql://`, `mysql+<driver>://`, `redshift://`, `snowflake://`, `sqlite://`) → URL form. Extract `db_type` from the scheme.
- **Bare path ending in `.db` / `.sqlite` / `.sqlite3`**, or starting with `/` or `~/` and not containing `://` → SQLite path. Resolve `~` to `$HOME`. Treat as `db_type = sqlite`.
- **Starts with `cloud-sql-proxy`** → recognize the Cloud SQL Auth Proxy idiom. Surface guidance and re-prompt:
  > That's the Cloud SQL Auth Proxy command. Two-step setup:
  > 1. In a separate terminal: paste and run that command. It opens a local port (default 5432, or whatever you passed via `--port`).
  > 2. Come back here and paste the URL you'd use to connect to that local port — typically `postgresql://USER:PASSWORD@127.0.0.1:<port>/DATABASE`. The `USER`, `PASSWORD`, and `DATABASE` are for your Cloud SQL database user, not the proxy.
  > Then I'll continue.

  Do not run the proxy command yourself — it's a long-running process and we don't manage its lifecycle.
- **Snowflake account locator alone** (e.g. `xy12345.us-east-1.aws`, no scheme, no path) → ask one follow-up: "I see a Snowflake account locator. Paste the full URL: `snowflake://USER:PASSWORD@<account>/<database>/<schema>?warehouse=<wh>&role=<role>`."
- **Anything else** → surface "I couldn't recognize that as a connection URL or a SQLite path. Paste a URL like `postgresql://...` or a path like `~/path/to.db`." Re-prompt.

### 0a.3 — Validate via the canonical parser

For URL inputs, validate the parse before writing:

```bash
python3 -c "
import sys; sys.path.insert(0, '$AGAMI_PLUGIN_ROOT/scripts')
from execute_sql import _parse_dsn
import json
print(json.dumps(_parse_dsn(sys.argv[1])))
" -- "<the URL>"
```

If `_parse_dsn` raises (unsupported scheme, missing required component), surface the error verbatim and re-prompt.

For SQLite paths, just check the file exists (or its parent dir exists if the user is creating a new DB).

### 0a.4 — Write credentials via the Write tool

**Use the Write tool, not Bash echo / heredoc** — Bash command lines render visibly in the chat host, leaking the password.

For URL form (postgres / mysql / redshift / snowflake):

```ini
# ~/.agami/credentials — written by agami-connect Phase 0a.
# Keep chmod 600. Add more profiles by appending another [section].
[main]
type = <postgres | mysql | redshift | snowflake>
url  = <pasted URL — verbatim, including +driver suffix and query params>
```
...
For appending to an existing credentials file (when the user is adding a second profile via `AGAMI_PROFILE=<new-name> /agami-connect`), Read the file first and append the new section — preserve existing sections.

### 0a.5 — chmod + auth files + .config

```bash
chmod 600 ~/.agami/credentials
python3 "$AGAMI_PLUGIN_ROOT/scripts/setup_pgauth.py" --profile main
```

`setup_pgauth.py` materializes `~/.agami/.pgpass` / `~/.agami/.mysql.cnf` / `~/.agami/.snowsql.cnf` from the credentials file so subsequent psql/mysql/snowsql calls don't put the password on the command line.

Write `~/.agami/.config` (JSON) with `active_profile = "main"` (or whatever the resolved `<profile>` was). If `.config` already exists, merge — don't overwrite tool_paths or analytics_consent if set.

### 0a.6 — Surface a one-line ack and continue

```
✓ Saved credentials for profile `main` (type: postgres). Continuing to introspect.
```

Then fall through to Phase 1 — no separate "init complete" handoff. The user pasted one URL and the introspect starts immediately.
