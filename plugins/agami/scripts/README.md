...
| `check_statement.py` | Checks one statement a person supplied, for `agami-reconcile` Phase 1.5: the read-only gate, the zero-row check, the statement, the semantic-model verbs and every probe, each execution through `execute_sql`'s guarded chokepoint, and writes the row directory the part ledger reads. Never runs anything on a command-line tier. | the agami-core package with the `model` extra, and the database's Python driver |
