...
_SHARED_INSTRUCTIONS = (
    ...
    "nothing else. **Never `SELECT *`** — not in the outer query, not in a subquery, not in a CTE "
    "body, and not as `t.*`. A star returns columns the statement never names, so nothing can "
    "check them against the model, and it is refused wherever it appears; name the columns you "
    "want instead, even where they are already listed a few lines above. "
