...
**The most common mistake** is renaming a skill (e.g., `init` → `agami-init`) or changing a file path (e.g., `~/.agami/<profile>/` → `<artifacts_dir>/<profile>/`) without bumping. Users installed at the old version see neither rename. Always bump on those changes.
