...
* an unqualified name declared under two or more schemas is refused as ambiguous, naming them;
...
def test_unqualified_name_declared_under_two_schemas_is_refused_as_ambiguous():
    ...
    assert refusal is not None
    assert refusal.rule == guardrail.RULE_TABLE_SCOPE
    assert "orders (in sales_data, staging)" in refusal.detail
...
def test_schemaless_and_schema_declarations_of_one_name_are_ambiguous_unqualified():
    ...
    assert refusal is not None and "orders (in no schema, staging)" in refusal.detail
