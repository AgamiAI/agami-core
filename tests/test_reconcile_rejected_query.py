...
def test_a_query_the_database_rejected_points_at_the_semantic_model(tmp_path: Path) -> None:
    item = _items(tmp_path, [_row(mode="mcp", probes=_trace())])[0]
...
def test_a_safety_check_block_counts_the_same_as_a_database_rejection(tmp_path: Path) -> None:
    ...
    item = _items(tmp_path, [_row(mode="mcp", probes=_trace(status="refused", detail=COLUMN_REFUSAL))])[0]
...
def test_a_tool_that_crashed_is_a_row_to_ask_again(tmp_path: Path) -> None:
    ...
    item = _items(tmp_path, [_row(mode="mcp", probes=_trace(status="raised", detail="KeyError"), error="KeyError")])[0]
...
def test_the_row_says_in_plain_words_what_stopped_the_query_and_what_to_do(tmp_path: Path) -> None:
    change = _items(tmp_path, [_row(mode="mcp", probes=_trace(status="refused", detail=COLUMN_REFUSAL))])[0]["change"]
...
def test_a_database_rejection_is_relayed_as_one(tmp_path: Path) -> None:
    change = _items(tmp_path, [_row(mode="mcp", probes=_trace())])[0]["change"]

    assert change[0] == "agami's query did not run. The database returned an error: " + REJECTION


@pytest.mark.parametrize("status, detail", [
    ("refused", "The result exceeded the 1000-row limit, so it was not returned."),
...
def test_a_query_stopped_for_a_reason_that_is_not_the_model_is_a_row_to_ask_again(
    tmp_path: Path, status: str, detail: str
) -> None:
    ...
    would be the wrong fix. The row still says what stopped the query."""
    item = _items(tmp_path, [_row(mode="mcp", probes=_trace(status=status, detail=detail), error=detail)])[0]

    assert item["fix"] == "ask_again" and item["owner"] == "agami"
    assert item["change"][0].startswith("agami's query did not run. ")
...
def test_one_missing_name_among_several_reasons_still_points_at_the_model(tmp_path: Path) -> None:
    ...
    item = _items(tmp_path, [_row(mode="mcp", probes=_trace(status="refused", detail=detail))])[0]
...
def test_a_graded_question_fit_does_not_hide_that_nothing_was_compared(tmp_path: Path) -> None:
    ...
    doubtful = _items(tmp_path / "a", [_row(mode="mcp", probes=_trace(), ledger=_fit_ledger("unresolved", "doubtful"))])[0]
    plausible = _items(tmp_path / "b", [_row(mode="mcp", probes=_trace(), ledger=_fit_ledger("confirmed", "plausible"))])[0]
...
def test_why_a_query_did_not_run_is_said_plainly_for_each_refusal() -> None:
    ...
    assert why("refused", "a rule nobody has named yet.") == blocked + "a rule nobody has named yet."
