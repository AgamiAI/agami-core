...
_RECURSIVE_ESCAPES = {
    "non_union_body": "WITH RECURSIVE secret AS (SELECT id FROM secret) SELECT id FROM secret",
    "anchor_arm": (
        "WITH RECURSIVE secret AS "
        "(SELECT id FROM secret UNION ALL SELECT id + 1 FROM secret WHERE id < 0) "
        "SELECT id FROM secret"
    ),
    "nested_in_non_union_body": (
        "WITH RECURSIVE secret AS (SELECT id FROM (SELECT id FROM secret) q) SELECT id FROM secret"
    ),
    "second_cte_of_a_recursive_clause": (
        "WITH RECURSIVE r AS (SELECT 1 AS n), secret AS (SELECT id FROM secret) "
        "SELECT id FROM secret"
    ),
    "inside_a_derived_table": (
        "SELECT id FROM (WITH RECURSIVE secret AS (SELECT id FROM secret) SELECT id FROM secret) z"
    ),
}
